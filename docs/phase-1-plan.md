...
- `docs/adaptations.md` is created with its first content: a pinned-interpretations register — every Phase 1 interpretation (the definition of done carries the exhaustive list) recorded with the SRD ambiguity and the chosen reading, so narrators and reviewers have one place to look instead of grepping docstrings. Documented adaptations proper (`Ruleset`-flagged deviations) join the same file in later phases.
...
- Every pinned interpretation in this plan is recorded in the `docs/adaptations.md` interpretations register, stated in a docstring, and locked by a test. The exhaustive list: multi-PR tiers carry no penalties; XP tiers evaluate best-first, first-match-wins; XP percentage results floor; HP re-roll repeats until ≥ 3; adjustment reductions are even per lowered score; gear combat-facet exemption from class weapon policy (including its magic-user consequence); basic-mode treasure-weight tracking with the general 1,600-coin cap; misc-gear flat 80 coins; ammunition weight 0; sling stones cost 0; INT-granted languages may not duplicate class natives.
