...
The player view is a safe projection: party status, explored map cells, known active effects, active quests (name, display narrative, and visible objectives with their states), and the journal. It never contains unexplored geometry, trap locations, monster HP, referee-only roll outcomes, session flags, hidden objectives, gate or trigger wiring, or the seed. `Visibility.REFEREE` returns everything, for LLM referees, debugging, and tests.
...
- Gates stay off specs that content packs embed. Packs are geometry-free, so the door and transition specs gates live on never ride in a pack — and a gate on pack-embeddable content would carry dangling item and flag references into arbitrary target adventures. Two further gate sites are designed but deferred with that pack interaction: trap-bypass conditions and condition-keyed description variants, the latter engine-resolved at emission time and shipped as text, because consumers cannot evaluate conditions by construction.
...
**Phase 11 — conditions, gates, and narrative blocks.** The condition union (`has_item` with consume-on-success, `flag_equals`, `effect_active`), `requires` on door and transition specs with the gate-refusal rejection codes and the lock-composition rules, and the narrative-block model in its full three-audience shape, defined at first use and reused unchanged by later phases. Milestone: a golden scenario in which a key-gated door refuses with authored text while consuming no draws and no time, opens once any member carries the key, a toll transition consumes on success, a locked-and-gated door requires both, and a door set open by referee command admits passage until it closes.
...
**Phase 15 — quests, victory, and the authored example.** The quest spec, the quest lifecycle commands with their state and player-visible events, adventure completion into victory, active quests in the player view, the per-level ambient guidance slot, and quest/objective reference validation; the example TUI crawler registers the interpreter, authors its fetch quest as adventure data, and deletes its hand-rolled listener; the osr-forge vocabulary-gap issue files as the phase's closing deliverable. Milestone: the crawler's fetch quest — activation, objective completion matched on the idol's catalog id, reward, journal beats, victory — runs end to end as authored data and replays clean.
