...
- The command set spans in-fiction commands (`MoveParty`, `Attack`, `CastSpell`) and referee-level commands (`GrantItem`, `AwardXP`, `SetFlag`, `SpawnMonsters`) — the surface a game's own logic, module scripting, the library's trigger/quest interpreter, or an LLM referee drives. Referee commands are logged and replayed like any other.
...
- Kernel functions, called à la carte, hold the same line for their own arguments: each one checks its arguments before its first draw and raises `ValueError` from the call, so a refused call costs no draws, no clock time, and no state, rather than drawing its way to a plausible result on an argument it should have refused.
...
- An adventure may also bundle **item templates** — the same weapon, armour, gear, and ammunition template models the equipment catalog ships, discriminated on the `item_type` field they already carry. Bundled templates join the shipped catalog for the adventure's sessions everywhere the engine resolves item ids, under the same contract as bundled monsters: colliding ids rejected at validation, never overridden. This is the item-identity surface for authored objects — a quest key or temple idol is an authored gear item, placed in caches, granted, and observed by catalog id through machinery that already speaks catalog ids. The town's shop is the one surface they do not reach: it stocks the shipped equipment lists, and bundled items enter play through caches and grants. Bundled item ids are adventure-scoped by construction: content-pack features reference the shipped catalog only, and pack insertion validates item references against the target adventure's effective catalog. Named valuables stay id-less generic treasure (a MacGuffin is an authored item in a cache, not a valuable), keeping the catalog the single item-identity namespace.
...
## Authored gates, triggers, and quests

An adventure can be won, and its content can be wired — the lever that opens the portcullis, the key that fits the lock, the idol the temple pays for — without the game writing code. Three layers: mechanics are two primitives, gates and triggers, with quests composed from both; narrative is authored text attached to those mechanical objects; presentation is whoever renders that text. The two primitives live at different depths of the engine, and that layering is a design commitment:

- **Gates are in-engine command-legality checks**, evaluated exactly the way `locked` is. They mutate nothing, so replay is unaffected.
- **The trigger/quest interpreter is a library-shipped listener that games register** — the documented extension contract, not a new in-session system. Everything the interpreter does lands in the command log as ordinary commands, and all quest, trigger, and journal state is engine-owned session state mutated only by those commands — so a replay, which runs with no listeners, rebuilds it exactly by re-executing the log. The interpreter holds no state the engine cannot reconstruct.

**Conditions.** A condition is a deterministic predicate over session state, evaluated live at the moment the gated thing is attempted — level-triggered, not edge-triggered, so a key dropped or sold stops opening doors and evaluation never drifts from the truth. Conditions form a discriminated union that grows additively; future kinds (NPC-related among them) arrive as new variants. The vocabulary:

- `has_item(item_id, consumes=False)` — some party member's carried inventory holds an item with that catalog id (equipment or magic item); any member's carried inventory is the whole test. With `consumes=True`, one instance — from the first holder in marching order — is removed when the gated command succeeds. Consumption is an effect of the successful command, reported through its events, never a side effect of evaluation, which stays pure.
- `flag_equals(key, value)` — a session flag holds a value.
- `effect_active(kind)` — an active effect of that kind is attached to a party member, letting authors choose between carrying the talisman and invoking it.

**Gates.** A gate is an optional `requires` condition on a door spec or a level-placed transition spec; failing it makes operating the door or taking the transition an illegal command. A gate guards an attempt, so only commands gate: a trap-effect transition (a chute or slide) is a forced relocation, not something the party attempts, and never carries `requires`. Gates and locks are orthogonal layers, deliberately not unified: `locked` is SRD-grounded, stateful, edge-triggered mechanics (thief skill rolls, per-character lockouts, overlay mutation on success), a gate is a stateless authored predicate, and merging them would force dice and character state into the condition union. The composition rules:

- A door carrying both requires both: lock and condition must each be satisfied.
- `PickLock` addresses only the lock — a thief is not a universal quest bypass. `SetDoorState` rewrites the door's state overlay only and never touches `requires`; specs stay immutable.
- A door standing open admits passage without a gate check. Triggers open gated doors by setting them open, not by clearing the gate; if the door closes again, the gate re-applies.
- A gate refusal is a command rejection with its own codes — `exploration.door.gate_refused`, `exploration.transition.gate_refused` — distinct from `exploration.door.locked`, carrying the gate's authored refusal text. Success text rides the normal door and transition events as an optional authored beat.
- Gates stay off specs that content packs embed, because a gate on pack-embeddable content would carry dangling item and flag references into arbitrary target adventures. Packs place no doors and no level transitions, and the one transition spec a pack can carry — inside a trap effect's slide — never gates by the attempt rule above, so no gate rides in a pack. Two further gate sites are designed but deferred with that pack interaction: trap-bypass conditions and condition-keyed description variants, the latter engine-resolved at emission time and shipped as text, because consumers cannot evaluate conditions by construction.

**Triggers.** A trigger is an authored binding from an observable event pattern, optionally further gated by conditions, to referee-command consequences — edge-triggered where gates are level-triggered. The observables are existing events (location entered, item acquired, monster defeated, flag set); the consequence surface is the existing referee command vocabulary. The semantics:

- Once-only by default; `repeatable` is the authored opt-in. Fired-state is engine session state, so once-only survives save/load and rebuilds under replay.
- Deterministic ordering: triggers matching one event fire in document order, a trigger's consequences execute in authored order, and all consequences of a batch execute before any subsequent player command.
- Bounded cascades: events produced by a player command are depth 0, events produced by a trigger's consequences are one deeper than the events that fired it, and triggers match events of depth 4 or less. Truncation records a referee-visibility note. Flag-chains are legitimate wiring — the bound is the runtime guarantee, and a cycle is suspicious authoring for an editor lint, not invalid content.
- Rejected consequences are dropped and recorded as referee-visibility notes — no retry and no queue, because deferred consequences firing later would be undebuggable. (A triggered ambush can collide with a wandering encounter opened by the same movement; the spawn drops and the note says so.)
- Trigger-granted treasure follows the normal valuation rules at the moment it lands: granted mid-dungeon it counts in the next return delta, granted at town it earns no treasure XP. Authored XP belongs in `AwardXP`.
- A consequence addresses characters through party selectors, never a literal id: `@party` expands at issue time to one command per living member in marching order, `@first` to the first living member, and validation rejects a document that names a character id, because character ids are allocated per session and no document can know one. The expansion happens as the commands issue, so the log stays concrete and replays exactly.
- The consequence surface is the referee command vocabulary minus three: the lifecycle commands (the interpreter's own vocabulary, never authored), `IdentifyItem` (its item ids are session-scoped instance ids, the same unknowability as character ids), and `RollDice` (a draw whose result no authored construct can read, so an authored roll would be a no-op that perturbs the adjudication stream).
- Monster-defeat events emit at battle end, so "the portcullis opens mid-fight as the boss falls" is not authorable; stated so authors' expectations are set.
...
**Narrative blocks.** Every mechanical object — quest, objective, gate, trigger — carries an optional narrative block with three audiences: display beats a deterministic renderer shows verbatim (offer, progress, and completion for quests; refusal and success for gates; fired text for triggers); a journal form; and LLM guidance that is never displayed verbatim. Guidance lives on the narrative block uniformly, plus one ambient guidance slot per dungeon level for steering that attaches to no mechanical object; it applies while its carrier is in play — quest active, gate encountered, trigger fired, level occupied — and is trusted as content, the same posture as description prose, which already flows into narration. A `speaker` attribution stays free prose; a typed NPC reference arrives additively if NPC entities ever land, and prose is never retrofitted into a reference.

**Visibility.** Player-visible events carry authored text; the wiring that produced them — flags, conditions, trigger internals — stays referee-only, because content wiring is the game's secret. Authored text on an event is content data in a structured field, not a violation of the no-baked-prose rule: events still carry message codes and never engine-baked English. Quest lifecycle, journal-entry, and adventure-completed events are player-visible; trigger-fired and note events are referee-visibility, as flag-set events already are.

**Victory and session end.** Completing a quest marked as concluding emits a player-visible adventure-completed event carrying the authored completion narrative and transitions the session to `victory` — a terminal mode mirroring game-over: play commands are illegal, referee commands remain legal except those that would resume play — teleporting the party, spawning an encounter — which is also what lets rewards land after the transition. The session concludes rather than continuing — the adventure module is the playable unit, campaign continuity is a non-goal, and a post-victory mode can arrive additively later where shipping "continue" first and tightening later could not. The same work closes an existing terminal-state gap: a non-battle party wipe (trap, deprivation) routes to game-over instead of leaving the session exploring forever.

**Validation.** `validate_adventure` resolves every authored reference: item ids in conditions, caches, and consequences against the effective catalog; monster, area, and dungeon/level references in triggers; the same references in every quest clause — activation, an objective's completion, a hidden objective's reveal — and in every reward. Quest and objective ids are checked where they are used rather than where they are written: no document authors a lifecycle command, because the authored-consequence type forecloses it at parse, so the four lifecycle commands resolve their ids against the adventure's quest specs at execution and reject an id no spec holds. Advisory analysis beyond hard reference checks — flag reads with no writer, trigger cycles, reachability — is authoring-tool territory, not engine validation.

Everything above is additive within the current `schema_version`: new optional fields, new event types, new save-state blocks, and the new session-mode value (see persistence, replay, and versioning for the enum-value rule).

Out of scope by decision, not omission: NPC entities and dialogue, branching narrative, quest accept/decline, post-victory play, procedurally generated quests, quest content in content packs, and campaign continuity. `Adventure.hooks` stays inert rumor prose — town color in a different register from quest offers, with no mechanical linkage from a rumor to a quest id.

## Extension points

The library implements B/X; game-design systems with no SRD basis — achievements, story progression, dialogue, and anything beyond the authored quest surface — belong to the game. What the library owes them is deterministic, serializable hooks:
...
A quest tracker is the canonical proof of this surface: the Phase 5 example crawler implements its fetch quest in its own game code — listeners, flags, referee commands — demonstrating that games need no library changes for game-design systems. The authored quest layer rides that proof rather than replacing it: the library's interpreter is an ordinary listener driving ordinary logged commands, first-party only because every adventure wants one. Phase 15 retires the example's hand-rolled listener by authoring the same quest as adventure data.
...
- Within a version, changes are additive only: new event types, new optional fields, and new values of serialized enums — an older artifact never contains the new value, and a newer artifact failing on an older reader is the same accepted risk as a new event type. Renames, removals, semantic changes, and changes to existing wire values bump the version.
...
Phases 10–15 deliver authored gates, triggers, and quests (see that section). The dependency chain is 10 → 11 → 14 → 15; phases 12 and 13 are order-independent before their consumers (12 before 15, 13 before 14).

**Phase 10 — adventure-bundled items.** Adventure documents carry item templates that join the equipment catalog for their sessions everywhere the engine resolves item ids, mirroring the Phase 9 monster contract; `validate_adventure` resolves item references — caches, grants — against the effective catalog. Milestone: a bundled gear item placed in a treasure cache is taken, granted, and observed by catalog id, and a colliding id is rejected at validation.

**Phase 11 — conditions, gates, and narrative blocks.** The condition union (`has_item` with consume-on-success, `flag_equals`, `effect_active`), `requires` on door and level-placed transition specs with the gate-refusal rejection codes and the lock-composition rules, and the narrative-block model in its full three-audience shape, defined at first use and reused unchanged by later phases; `validate_adventure` grows condition item-id checks. Milestone: a golden scenario in which a key-gated door refuses with authored text while consuming no draws and no time, opens once any member carries the key, a toll transition consumes on success, a locked-and-gated door requires both, and a door set open by referee command admits passage until it closes.

**Phase 12 — terminal modes.** `SessionMode.VICTORY` with its command-legality rules, and non-battle party wipes routing to `GAME_OVER` — the terminal-state machinery opened once for both changes; victory becomes reachable in Phase 15. Milestone: a trap or deprivation wipe ends the session in game-over, and mode-legality tests hold for victory.

**Phase 13 — journal and lifecycle commands.** The engine substrate with no interpreter: the optional `source` field on commands, session-state blocks for trigger fired-marks and the journal, `MarkTriggerFired`, `AddJournalEntry`, and `RecordNote` with their events, the journal in the player view, and persistence of the new blocks. Milestone: save/load and replay rebuild journal and fired-marks exactly, with no listeners registered, and `source` survives the command log.

**Phase 14 — triggers and the interpreter.** The trigger spec, authored triggers on the adventure document, and the library-shipped interpreter listener: document-order matching, once-only fired-state, the depth-4 cascade bound with truncation notes, and dropped-consequence recording; `validate_adventure` grows trigger reference checks. Milestone: the lever-opens-portcullis scenario authored as data replays identically with no listeners, and a trigger spawn colliding with an open encounter drops and records its note.

**Phase 15 — quests, victory, and the authored example.** The quest spec, the quest lifecycle commands with their state and player-visible events, adventure completion into victory, active quests in the player view, the per-level ambient guidance slot, and quest/objective reference validation; the example TUI crawler registers the interpreter, authors its fetch quest as adventure data, and deletes its hand-rolled listener. Closing deliverable: file the osr-forge issue recording that its overrides schema has no quest surface — quest authoring stays native-project-only until osr-forge grows one in its own repo against this settled schema. Milestone: the crawler's fetch quest — activation, objective completion matched on the idol's catalog id, reward, journal beats, victory — runs end to end as authored data and replays clean.
