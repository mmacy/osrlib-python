...
**Validation.** `validate_adventure` resolves every authored reference: item ids in conditions, caches, and consequences against the effective catalog; monster, area, and dungeon/level references in triggers; quest and objective ids in lifecycle references. Advisory analysis beyond hard reference checks — flag reads with no writer, trigger cycles, reachability — is authoring-tool territory, not engine validation.
