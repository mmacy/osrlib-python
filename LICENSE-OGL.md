...
The files under `srd/` are the scraped [Old-School Essentials System Reference Document](https://oldschoolessentials.necroticgnome.com/srd/) and are Open Game Content under the Open Game License 1.0a, reproduced below with its complete Section 15 copyright notice as published by Necrotic Gnome. The same license will apply to `osrlib/data/`, the machine-readable content compiled from `srd/`, when it exists; an osrlib entry will be added to Section 15 when compiled Open Game Content is first distributed.
