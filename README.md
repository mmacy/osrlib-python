...
Early development — Phase 3 (magic) is complete: the spell compiler (all 106 SRD spell pages behind the cleric and magic-user lists, with structured modes, targeting, saves, and effects for the automated subset and tagged prose for the rest), spell books and memorization for divine and arcane casters, casting with the no-leak eligibility doctrine, spell disruption, turning undead with the verbatim turning table, stat-modifier buffs and wards riding the effects engine (*bless*, *shield*, *protection from evil*, *resist fire*, *striking*), and spell durations — per-level arithmetic included — on the Phase 2 ledger. Phase 2 delivered the combat kernel: the monster compiler (all 138 SRD monster pages, with packed-variant expansion, structured defenses, and ability tags), the combat tables as data, attack resolution with weapon-material and graded-immunity damage, the effects engine, initiative, morale, saving throws, and energy drain. Phase 1 delivered characters (classes, ability scores, creation, inventory and encumbrance, XP and leveling) and Phase 0 the contracts underneath (deterministic named RNG streams, the dice grammar, the game clock, the event-emission rules). The battle state machine and the dungeon crawl arrive in later phases.
